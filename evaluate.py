...
def preprocess(dataset_files, batch_size, hdf5_dest_path=None):
    ...
    if hdf5_dest_path and os.path.exists(hdf5_dest_path):
        with tables.open_file(hdf5_dest_path, 'r') as file:
            ...
            in_data = list(zip(features, features_len, transcript, transcript_len))
...
def calculate_report(labels, decodings, distances, losses):
    ...
    total_levenshtein = sum(map(attrgetter('levenshtein'), samples))
    total_label_length = sum(map(attrgetter('label_length'), samples))
...
def main(_):
    ...
    test_data = preprocess(FLAGS.test_files, FLAGS.test_batch_size, hdf5_dest_path=FLAGS.hdf5_test_set).sort_values(by="features_len", ascending=False)
    ...
    with tf.Session() as session:
        ...
        decoded, _ = decode_with_lm(decode_logits_ph, seq_lengths_ph, merge_repeated=False, beam_width=FLAGS.beam_width)

        sparse_labels = tf.cast(ctc_label_dense_to_sparse(labels_ph, label_lengths_ph, FLAGS.test_batch_size), tf.int32)
        loss = tf.nn.ctc_loss(labels=sparse_labels, inputs=decode_logits_ph, sequence_length=seq_lengths_ph)
        ...
        bar = progressbar.ProgressBar(max_value=batch_count-1, widget=progressbar.AdaptiveETA)
        ...
        for batch in bar(split_data(test_data, FLAGS.test_batch_size)):
            ...
            for i in range(0, features.shape[1], N_STEPS):
                ...
                output = session.run(outputs['outputs'], feed_dict={
                    inputs['input']: chunk,
                    inputs['input_lengths']: np.minimum(features_len, full_step_len)
                })
                logits = np.concatenate((logits, output))

                # we have processed N_STEPS so subtract from remaining steps
                features_len -= N_STEPS
                # clip to zero
                features_len = np.maximum(features_len, np.zeros_like(features_len))
        ...
        # Order samples by their loss (lowest loss on top)
        samples.sort(key=lambda s: s.loss)
        ...
        print('Test - WER: %f, loss: %f, mean edit distance: %f' % (wer, mean_loss, mean_edit_distance))
        if len(samples) > 0:
            print('-' * 80)
